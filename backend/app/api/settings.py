...
from app.services.settings_service import DEFAULTS, all_settings, get_setting, provider_quotas, set_setting
...
def data_sources(db: Session = Depends(get_db)):
    ...
    quotas = provider_quotas(db)  # latest remaining/used credits as reported by the provider itself
    return {
        "mode": s.app_mode,
        ...
        "providers": [
            ...
            {"key": "the_odds_api", "name": "The Odds API", "role": "odds", "configured": status["the_odds_api"], "active": not s.is_demo and status["the_odds_api"] and s.odds_provider == "the_odds_api", "fields": ["1X2", "totals", "BTTS", "team totals", "spreads", "1H totals"], "notes": "No corner markets. Free tier 500 credits/month.", "quota": quotas.get("the_odds_api")},
